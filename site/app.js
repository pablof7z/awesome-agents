(function () {
  ...
  function sparkline(data) {
    ...
    return '<svg class="spark" viewBox="0 0 ' + w + ' ' + h + '" preserveAspectRatio="none" aria-hidden="true">' +
      ...
      '<circle class="dot" cx="' + last[0] + '" cy="' + last[1] + '" r="1.9"/></svg>';
  }

  function matches(a, q) {
    if (!q) return true;
    var hay = (a.name + " " + a.slug + " " + a.desc + " " + a.role + " " + a.tags.join(" ")).toLowerCase();
  ...
  function rowHTML(a, i) {
    ...
    var empty = a.examples > 0 ? "" :
      '<p class="agent-empty">No public samples yet, but the definition is open. Read it before you install. ' +
        '<a href="/agents/' + esc(a.slug) + '/definition">View definition &rarr;</a></p>';
    var top = i < 3 ? ' data-top="' + (i + 1) + '"' : "";
    return '<div class="trow" role="row"' + top + ' data-href="/agents/' + esc(a.slug) + '" title="Open ' + esc(a.name) + '">' +
      ...
      '<div class="cell-main" role="cell">' +
        ...
        '<p class="agent-desc"><b class="role-noun">' + esc(a.role) + "</b> " + esc(a.desc) + "</p>" +
        '<span class="agent-meta">' + proof + "</span>" +
        empty +
      "</div>" +
  ...
  function render(animate) {
    ...
    emptyEl.hidden = true;
    rowsEl.classList.toggle("animate", !!animate && !reduce);
  ...
  var totalInstalls = agents.reduce(function (s, a) { return s + a.installs; }, 0);
  countUp(document.getElementById("stat-count"), agents.length, function (n) { return String(n); });
  countUp(document.getElementById("stat-installs"), totalInstalls, fmt);
  ...
  // ---- reveal sections + run the hero terminal ----
  var term = document.getElementById("term-body");
  if (term && !reduce) requestAnimationFrame(function () { term.classList.add("run"); });
  else if (term) term.classList.add("run");
