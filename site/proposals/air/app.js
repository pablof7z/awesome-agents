// Shared proposal app.js — same behavior as the live site, plus optional
// trust affordances (version, publisher, vouches) read from the row data.
// Proposals opt in via window.AA_TRUST = { version, publisher, vouches }.
(function () {
  "use strict";
  var agents = window.AGENTS || [];
  var trust = window.AA_TRUST || {};
  ...
  function sparkline(data) {
    ...
    var span = max - min || 1; var step = (w - pad*2) / (data.length - 1);
    var pts = data.map(function (d, i) {
      var x = pad + i*step, y = h - pad - ((d-min)/span)*(h-pad*2);
  ...
  // ---- trust affordances (opt-in) ----
  function trustBits(a) {
    if (!trust.on) return "";
    var bits = [];
    if (trust.version && a.version) bits.push('<span class="t-ver">v' + esc(a.version) + "</span>");
    if (trust.publisher && a.publisher) bits.push('<span class="t-pub">' + esc(a.publisher) + "</span>");
    if (trust.vouches && a.vouches && a.vouches.length) {
      var pills = a.vouches.slice(0, 3).map(function (v) {
        var me = v === "pablof7z" ? " me" : "";
        return '<span class="t-vouch' + me + '">' + esc(v) + "</span>";
      }).join("");
      bits.push('<span class="t-vouches" title="vouched by ' + esc(a.vouches.join(", ")) + '">' + pills + "</span>");
    }
    return bits.length ? '<span class="trust">' + bits.join("") + "</span>" : "";
  }
  ...
  function rowHTML(a, i) {
    ...
    return '<div class="trow" role="row" data-copy="' + esc(cmdFor(a)) + '" title="Click to copy install command">' +
      ...
      '<div class="cell-main" role="cell">' +
        ...
        '<span class="agent-meta">' + proof + trust + "</span>" + empty +
      "</div>" +
  ...
  function render() {
    ...
    if (!list.length) { rowsEl.innerHTML = ""; emptyQEl.textContent = state.q; emptyEl.hidden = false; return; }
    emptyEl.hidden = true;
  ...
  function showToast(msg) {
    ...
    clearTimeout(showToast._t); showToast._t = setTimeout(function () { toast.classList.remove("show"); }, 1800);
  }
  ...
  document.addEventListener("click", function (e) {
    ...
    copyText(btn.getAttribute("data-copy")).then(function () {
      btn.classList.add("copied"); showToast("Copied install command");
  ...
  tabs.forEach(function (tab) {
    tab.addEventListener("click", function () {
      ...
      state.sort = tab.getAttribute("data-sort"); render();
    });
  });
  render();
  ...
  (function roster() {
    ...
    setInterval(function () {
      ...
      el.style.opacity = "0";
      setTimeout(function () { el.textContent = phrases[idx]; el.style.opacity = "1"; }, 260);
    }, 3000);
  })();
})();